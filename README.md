...
Current release: [v2.8.11](https://github.com/yeager/zscaler-api-client/releases/tag/v2.8.11).
...
- **PAC mappings** – Correlate ZIA PAC metadata with every supplied ZCC forwarding-profile action by exact hosted URL or inline PAC fingerprint, highlighting confirmed, inline, unconfigured, and unresolved references without exposing PAC content or guessing by name.
- **Data-center-aware PAC guidance** – Ship a searchable, locally indexed Zscaler Cloud Enforcement Node catalog (CIDRs, proxy/VPN hostnames, GRE and extranet virtual IPs). Hover a PAC line for an explanatory balloon; explicit Zscaler endpoints are correlated to their data center and region without sending PAC content externally. Refresh the source index with `python3 scripts/update_zscaler_config_index.py`.
...
- **Exposure, access and investigation notebook** – Analyze complete response trees for explicit exposure and broad privileges, present guarded defensive-deception ideas, and keep masked tenant-isolated analyst notes without deploying controls
